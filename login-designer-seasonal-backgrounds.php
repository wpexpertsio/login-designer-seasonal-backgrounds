...
if ( ! Login_Designer_Seasonal_Backgrounds::is_login_designer_active() ) {

	/**
	 * Render admin notice.
	 */
	function login_designer_seasonal_backgrounds_admin_notice() {

		$url = esc_url( add_query_arg( array(
			'utm_source'   => 'plugins-page',
			'utm_medium'   => 'plugin-admin-bail-notice',
			'utm_campaign' => 'admin',
			'utm_content' => 'login-designer-seasonal-backgrounds',
			),
		'https://logindesigner.com/' ) ); ?>

		<div class="error">
			<p><?php echo sprintf( esc_html__( 'Install %s to activate the Login Designer Seasonal Backgrounds extension.', '@@textdomain' ), '<a href="' . esc_url( $url ) . '">Login Designer</a>' ); ?></p>
		</div>

		<?php

	}
	add_action( 'admin_notices', 'login_designer_seasonal_backgrounds_admin_notice' );
